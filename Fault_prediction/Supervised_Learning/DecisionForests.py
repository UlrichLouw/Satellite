...
def DecisionTreeAllAnomalies(path, depth, multi_class = False, constellation = False, lowPredictionAccuracy = False, MovingAverage = True, includeAngularMomemntumSensors = False, includeModelled = False):
    X_list = []
    Y_list = []
    ...
    if lowPredictionAccuracy:
        pickle.dump(clf, open(path + '/DecisionTreesPhysicsEnabledDMDBinaryClassLowAccuracy' + str(SET_PARAMS.treeDepth) + '.sav', 'wb'))
        if SET_PARAMS.Visualize:
            fig = plt.figure(figsize=(25,20))
            tree.plot_tree(clf, feature_names = ColumnNames, filled=True, max_depth = 2, fontsize = fontsize)
            fig.savefig(path + '/DecisionTreeBinaryClassLowAccuracy' + str(SET_PARAMS.treeDepth) + '.png')
    elif multi_class and constellation:
        #! ClassNames = ["First", "Second", "Third", "Fourth", "Fifth"]
        pickle.dump(clf, open(path + '/ConstellationDecisionTreesPhysicsEnabledDMDMultiClass' + str(SET_PARAMS.treeDepth) + '.sav', 'wb'))
        if SET_PARAMS.Visualize:
            fig = plt.figure(figsize=(25,20))
            tree.plot_tree(clf, feature_names = ColumnNames, filled=True, max_depth = 2, fontsize = fontsize)
            fig.savefig(path + '/ConstellationDecisionTreeMultiClass' + str(SET_PARAMS.treeDepth) + '.png')
    elif constellation:
        pickle.dump(clf, open(path + '/ConstellationDecisionTreesPhysicsEnabledDMDBinaryClass' + str(SET_PARAMS.treeDepth) + '.sav', 'wb'))

        if SET_PARAMS.Visualize:
            fig = plt.figure(figsize=(25,20))
            tree.plot_tree(clf, class_names = ClassNames, feature_names = ColumnNames, filled=True, max_depth = 2, fontsize = fontsize)
            fig.savefig(path + '/ConstellationDecisionTreeBinaryClass' + str(SET_PARAMS.treeDepth) + '.png')

    elif multi_class:
        pickle.dump(clf, open(path + '/DecisionTreesPhysicsEnabledDMDMultiClass' + str(SET_PARAMS.treeDepth) + '.sav', 'wb'))
        if SET_PARAMS.Visualize:
            fig = plt.figure(figsize=(25,20))
            tree.plot_tree(clf, class_names = ClassNames, feature_names = ColumnNames, filled=True, max_depth = 2, fontsize = fontsize)
            fig.savefig(path + '/DecisionTreeMultiClass' + str(SET_PARAMS.treeDepth) + '.png')
    else:
        pickle.dump(clf, open(path + '/DecisionTreesPhysicsEnabledDMDBinaryClass' + str(SET_PARAMS.treeDepth) + '.sav', 'wb'))

        if SET_PARAMS.Visualize:
            fig = plt.figure(figsize=(25,20))
            tree.plot_tree(clf, class_names = ClassNames, feature_names = ColumnNames, filled=True, max_depth = 2, fontsize = fontsize)
            fig.savefig(path + '/DecisionTreeBinaryClass' + str(SET_PARAMS.treeDepth) + '.png')
